...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    for (let g = 0; g < groupCount; g++) {
      const groupTeams = createdTeams.slice(g * teamsPerGroup, (g + 1) * teamsPerGroup);
    ...
    await tournament.save();
    await tournament.populate("teams groups.teams groups.matches.team1 groups.matches.team2");
...
router.post("/:id/matches", authenticateToken, async (req, res) => {
  try {
    ...
    await tournament.save();
    await tournament.populate("groups.teams groups.matches.team1 groups.matches.team2");
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    // Teams des Turniers ebenfalls löschen
    await Team.deleteMany({ tournamentId: req.params.id });
    await Tournament.findByIdAndDelete(req.params.id);

    res.json({ message: "Turnier erfolgreich gelöscht" });
  } catch (error) {
    console.error("❌ Löschfehler:", error);
    res.status(500).json({ message: "Fehler beim Löschen des Turniers" });
  }
});

// ✅ WICHTIG: Router exportieren
